    ...
    <div class="col-12">
      @if(count($recipe_categories) > 0)
        <div class="card-header">
          <h3 class="text-hover">Recipe Categories</h3>
        </div>
          @foreach($recipe_categories->take(10) as $category)
            <a href="{{ route('web_recipe_category.show',['id' => $category->id]) }}">
              <div class="row">
                          ...
                          <div class="row">
                            <div class="col-xs-12 col-sm-12 col-lg-md-12 col-lg-12 text-center">
                              <img class ="category-image img-responsive img-thumbnail m-0 p-0 w-100"style="height: 100px; width: 100%;" 
                                src="{{ URL::asset('assets/images/recipes/category/'.$category->avatar) }}"
                                 alt=""
                              >
                            </div>
                          </div>
        ...
        <div class="row pt-1 pb-2">
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <a class="btn btn-lg  btn-outline-light btn-block" href="{{ route('web_recipe_category.index') }}"><strong>Find Out More</strong></a>
          </div>
        </div>
      @endif
    </div>
