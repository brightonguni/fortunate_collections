...
<div id="package-carousel" class="carousel slide">
  ...
  {{-- <ol class="carousel-indicators">
    @foreach($packages as $slide)
      <li data-target="#package-carousel" data-slide-to="{{ $loop->index }}" class="{{ $loop->first ? 'active' : '' }}"></li>
    @endforeach
  </ol> --}}
      <div class="carousel-inner m-0 p-0 container-overlay">
        @foreach($blogs as $slide)
          <div class="carousel-item slide-item-image {{ $loop->first ? ' active' : '' }} fade-in" style="background-image: url('{{ asset('assets/images/packages/'.$slide->avatar)}}');" 
      ...
      <a class="left carousel-control" href="#package-carousel" data-slide="prev">
        <span class="glyphicon glyphicon-chevron-left"></span>
        <span class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" href="#package-carousel" data-slide="next">
