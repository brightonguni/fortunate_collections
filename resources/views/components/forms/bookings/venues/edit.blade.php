<form class="pt-2" data-async data-target="#edit" id="BookingVenueDetailsForm" enctype="multipart/form-data"
    action="/booking/venue{{$venue->id}}" novalidate _lpchecked="1" method="POST" enctype="multipart/form-data">
    @csrf
    <input type="text" name="venue_id" value="{{ $venue->id }}" hidden="hidden">
    <div class="row">
        <div class="col-md-9">
            <div class="form-row">
                <div class="form-group col-12 m-0 pt-3">
                    <label for="name" class="col-form-label s-12">title *</label>
                    <input id="title" name="title" autofocus placeholder="{{ $venue->title }}" value="{{ $venue->name }}"
            ...
            <div class="form-row">
                <div class="form-group col-12 m-0 pt-3">
                    ...
                    <textarea style="height: 145px;" required class="textarea form-control  @if($errors->first('description')) is-invalid  @endif r-0" name="description" id="description" cols="10" rows="9"
                              placeholder="{{ $venue->description }}">{{ $venue->description }}
                    </textarea>
                    @if($errors->any())
                    <div class="invalid-feedback">
          ...
          <div class="form-row">
              @if( !isset($venue->activate) )
              <div class="form-group col-12 m-0 pt-3 ">
                  <label class="my-1 mr-2" for="active">Publish</label>
                  <div class="material-switch">
                      <input id="active" {{($venue->active)? 'checked': '' }} name="active" type="checkbox">
          ...
          @endif
            ...
            @if( isset($venue->activate) )
                <div class="form-group col-12 m-0 pt-3 ">
                    ...
                    <div class="material-switch">
                        <input id="active"
                               {{($venue->activate)? 'checked': '' }}  name="active"
                               type="checkbox">
                        <label for="active" >Yes</label>
                    </div>
                </div>
