<div class="single-info-details pt-5">
  <div class="item-img">
    <img style="padding-right: 15px;" 
        src="{{asset('assets/images/recipes/method/'.$recipe_method->avatar)}}" 
        alt="no recipe image"
  </div>
        ...
        <ul>
          <ul>
            @if(in_array('recipe_method_update', $canDo))
              <li><a href="/recipe-method/{{ $recipe_method->id }}/edit"><i class="far fa-edit"></i></a></li>
            @endif
            @if(in_array('recipe_method_delete', $canDo))
              <li>
                <a 
                  class="dropdown-item" 
                  data-target-id="{{$recipe_method->id}}" 
                  data-toggle="modal" 
                  data-target="#delete"
                >
    ...
    <div class="info-table table-responsive">
      <table class="table text-nowrap">
        <tbody>
          ...
          <tr>
            <td>Status:</td>
            <td class="font-medium text-dark-medium">{{ $recipe_method->active }}</td>
          </tr>
