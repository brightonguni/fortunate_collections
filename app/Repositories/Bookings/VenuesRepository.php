 ...
 */
    ...
    {
        if (self::is_valid()) {
            $venue_avatar = request()->file('avatar');
            if ($venue_avatar) {

                $f_name = time() . '.' . $venue_avatar->getClientOriginalExtension();
                $location = public_path('assets/images/bookings/venue/' . $f_name);
                Image::make($venue_avatar)->resize(300, 300)->save($location);
                $data['avatar'] = $f_name;
            }
            $venue = Venue::create([
                'avatar' => $f_name,
            ...
            return redirect()
                ->route('booking_venue.show', [$venue->id])
