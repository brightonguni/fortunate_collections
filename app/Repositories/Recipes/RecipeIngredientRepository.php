...
use App\Entities\Recipes\ingredient\Form;
...
class RecipeIngredientRepository extends Form implements RepositoryInterface
{
    use Statistics;
 ...
 */
    ...
    public function all()
    {
        ...
        switch ($role) {
            case "1":
                return Ingredient::all()->sortBy('created_at');
                break;
            case "2":
                return [];
                break;
            ...
            case "4":
                ...
                return Ingredient::whereIn('store_id',
                    $licensor_stores)
                    ->get()
                    ->sortBy('created_at');
                break;
    ...
    public function store(array $data)
    {
        if (self::is_valid()) {
            ...
            $recipe_ingredient = Ingredient::create([
            ...
            return redirect()
                ->route('recipe_ingredient.show', [$recipe_ingredient->id])
                ->with('success', 'You have successfully create a new recipe ingredient' . $data['recipe_id']);
        }
        return redirect()
            ->back()
    ...
    public function update($id, array $data)
    {
        if (self::is_valid()) {
            ...
            return redirect('recipe-ingredient/' . $id)->with('success', 'Recipe Ingredient updated successfully');
        }

    }

    public function getActiveRecipeIngredient()
    {
        $active_recipes = App::make(RecipeIngredientRepository::class)->all()
            ->where('active', '1')
            ->count();
