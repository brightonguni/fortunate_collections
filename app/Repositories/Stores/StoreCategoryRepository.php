...
use App\Entities\Stores\Categories\Form;
...
use Image;
class StoreCategoryRepository extends Form implements RepositoryInterface
{
    use Statistics;
    /**
    ...
    public function all()
    {
        return StoresCategory::all()->sortByDesc('created_at');
    }
...
//        Session::flash('errors',self::errors());
    ...
    {
        if (self::is_valid()) {
            $store_category_avatar = request()->file('avatar');
            if ($store_category_avatar) {

                $f_name = time() . '.' . $store_category_avatar->getClientOriginalExtension();
                $location = public_path('assets/images/store/category/' . $f_name);
                Image::make($store_category_avatar)->resize(400, 400)->save($location);
                $data['avatar'] = $f_name;
            }

            $category = StoresCategory::create([
                'title' => $data['title'],
            ...
            return redirect()
                ->route('stores.categories.show', [$category->id])
                ->with('success', 'You have successfully create contacts' . $data['title']);
        }
        return redirect()
            ->back()
    ...
    {
        return Licensor::all()->pluck('name', 'id');
    }
    public function getStores()
    {
        return Stores::all()->pluck('name', 'id');
    }
    public function getStatistics()
    {
        return (object) [
            ...
            'total' => $this->getDeletedTotal(self::all()),
        ];
    }
