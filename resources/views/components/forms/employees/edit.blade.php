...
<form class="pt-2 " data-async data-target="#edit" id="userProfileForm" enctype="multipart/form-data" action="{{ route('users.update', ['id' => $employee->id]) }}" novalidate
      _lpchecked="1" method="POST">
    @csrf
    <input type="hidden" name="csrf-token" value="_token">
            ...
            <div class="col-md-12">
                <div class="form-row">
                    <div class="form-group col-12 m-0 pt-3">
                        ...
                        <input id="email" name="email" placeholder="{{ $employee->email }}"
                ...
                <div class="form-row">
                    <div class="form-group col-12 m-0 pt-3">
                        ...
                        <input id="password" name="password" placeholder="Please enter your password"
                               ...
                               class="form-control r-0 light s-12  @if($errors->first('password')) is-invalid  @endif">
                        @if($errors->any())
                            <div class="invalid-feedback">
                ...
                <div class="form-row">
                    <div class="form-group col-6 m-0 pt-3">
                        <label for="first_name" class="col-form-label s-12">First
                            Name</label>
                        <input id="first_name" name="first_name" autofocus
                               placeholder="{{ $employee->first_name }}"
                               value="{{ $employee->first_name }}"
                               maxlength="50"
                               class="form-control r-0 light s-12  @if($errors->first('first_name')) is-invalid  @endif"
                               required type="text">
                        @if($errors->any())
                            <div class="invalid-feedback">
                    ...
                    <div class="form-group col-6 m-0 pt-3">
                        <label for="last_name" class="col-form-label s-12 ">Last
                            Name</label>
                ...
                <div class="form-row mt-1">
                    <div class="form-group col-6 m-0 pt-3">
                        <label for="phone" class="col-form-label s-12">Phone
                            Number</label>
                        <input id="phone" placeholder="{{ $employee->phone }}"
                               value="{{ $employee->phone }}" required name="phone"
                               maxlength="10"
                    ...
                    <div class="form-group col-6 m-0 pt-3">
                        ...
                        <select required name="role_id"
                                ...
                                id="role" data-roles="{{ json_encode($roles) }}">
                            @foreach($roles as $role)
                                <option
                                    {{($role->name == $employee->role()->name)? 'selected' : ''}} value="{{$role->id }}">{{ $role->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @if($errors->any())
                ...
                <div class="form-row mt-1">
                     <div class="form-group col-6 m-0 pt-3">
                        <label class="my-1 mr-2" for="role">Licensor </label>
                        <select required name="licensor_id"
                                class="custom-select my-1 mr-sm-2 form-control r-0 light s-12"
                                id="licensor_id" data-licensors="{{ json_encode($licensors)}}">
                            @foreach($licensors as $licensor)
                                <option
                                    {{($licensor->name == $employee->licensor->name)? 'selected' : ''}} value="{{$licensor->id }}">{{ $licensor->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @php
                    $error ='hidden-error';
                    if($errors->has('store') || $employee->store){
                        $error = '';
                    }
                    @endphp
                    <div class="form-group col-6 m-0 pt-3 {{ $error }}" id="stores_area">
                        <label class="my-1 mr-2" for="role">Stores </label>
                        <select required name="store"
                                class="custom-select my-1 mr-sm-2 form-control r-0 light s-12"
                                id="store_id" data-stores="{{ json_encode($stores) }}">
                           <option value="">Select Store</option>
                           @foreach($stores as $store)
                            @if($employee->store)
                                @if($employee->store->store_id == $store->id)
                    ...
                    <div class="form-group col-12 m-0 pt-3">
                      ...
                      <select 
                        name="project_id" 
                        required class="custom-select my-1 mr-sm-2 form-control r-0 light s-12" 
                        id="project_id"
                      >
                        @foreach($projects as $project)
                          @if($employee->project)
                              <option value="{{$project->id}}">{{ $project->name }}</option>
                                  @else
                                  <option {{($employee->name == $employee->project->name)? 'selected' : ''}}  value="{{$project->id}}">{{ $project->name }}</option>
                              @endif
                          @endforeach
                      </select>
                  </div> 
                      ...
                      @foreach($teamss as $team)
                        @if($employee->team)
                            <option value="{{$team->id}}">{{ $team->name }}</option>
                                @else
                                <option {{($team->name == $team->project->name)? 'selected' : ''}}  value="{{$team->id}}">{{ $team->name }}</option>
                            @endif
                        @endforeach
                    </select>
                </div> 
...
<script type="text/javascript">
    $(document).ready(function(){
        var roleval = null
        var activebusiness = <?php echo json_encode($employee->store) ?>;
        ...
        var role = <?php echo $employee->role()->id ?>;
