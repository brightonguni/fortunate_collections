...
<div class="card height-auto" id="index-card">
  <div class="card-body">
    <form class="pt-2"  id="createProductColourForm" action="{{route('product_color.update',['id' => $color->id])}}" 
        method="POST" enctype="multipart/form-data" novalidate_lpchecked="1"
    >
      @csrf
       <input type="text" name="color_id" value="{{ $color->id }}" hidden="hidden">
      <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-3 col-lg-3 pt-4">
          {{-- <div class="col-md-12 service-avatar pl-0"> --}}
            <input  type="file" id="avatar" name="avatar" value="{{ $color->avatar }}">
        ...
        <div class="col-xs-12 col-sm-12 col-md-9 col-lg-9">
          <div class="form-row">
            <div class="form-group col-12 m-0 pt-3">
              ...
              <input id="name" name="name" autofocus placeholder="{{ $color->name }}"
                value="{{ $color->name }}" maxlength="191"  required type="text"
                class="form-control 
                r-0 light s-12  
